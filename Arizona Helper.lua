...
function initialize_commands()
	...
	sampRegisterChatCommand("activate", function() 
		if thisScript().version:find('VIP') then
			...
			sampAddChatMessage('[Arizona Helper] {ffffff}Перейдите в Telegram/Discord VIP бота (@смениюз), и активируйте ключик', message_color) 
...
function send_no_vip_msg()
	for i = 1, 1, 1 do
		sampAddChatMessage('[Arizona Helper] {ffffff}Вы попытались использовать функционал, который недоступен в FREE версии! Купите подписку LUAVIP!', message_color)
	end
end
...
function check_update()
	...
	downloadFileFromUrlToPath('https://github.com/luamods/arizona-helper/raw/refs/heads/main/Update.json', configDirectory .. "/Update.json")
end
function check_resourses()
	...
	if not doesFileExist(configDirectory .. '/Resourse/logo.png') then
		...
		downloadFileFromUrlToPath('https://github.com/luamods/arizona-helper/blob/main/Resourse/logo.png', configDirectory .. '/Resourse/logo.png')
	...
	if not doesFileExist(configDirectory .. "/Resourse/notify.mp3") then
		...
		downloadFileFromUrlToPath('https://github.com/luamods/arizona-helper/blob/main/Resourse/notify.mp3', configDirectory .. "/Resourse/notify.mp3")
	end
...
function sampev.onSendGiveDamage(playerId, damage, weapon, bodypart)
	...
	if playerId ~= 65535 then
		if (sampGetPlayerNickname(playerId) == 'Nester_Ryancountras' and getServerNumber() == '20') or sampGetPlayerNickname(playerId):find('%[20%]Nester_Ryancountras') then
			sampAddChatMessage('[Arizona Helper] {ffffff}Nester_Ryancountras - это разработчик Arizona Helper!', message_color)
...
function sampev.onServerMessage(color, text)
	...
	if (text:find('Nester_Ryancountras') and getServerNumber() == '20') or text:find('%[20%]Nester_Ryancountras') then
		...
		elseif text:find('Nester_Ryancountras%[%d+%]') then
			...
			text = string.gsub(text, 'Nester_Ryancountras%[%d+%]', message_color_hex .. 'LUAMODS[' .. id .. ']' .. lastColor)
			...
			if imgui.BeginTabItem(fa.GEAR..u8' Настройки') then 
				if imgui.BeginChild('##1', imgui.ImVec2(589 * settings.general.custom_dpi, 187 * settings.general.custom_dpi), true) then
					...
					if imgui.SmallButton(u8'Discord') then
						openLink('https://discord.gg/6KAf8HjNSj')
					end
					imgui.SameLine()
					imgui.Text('/')
					imgui.SameLine()

...
local commands = {
	commands = {
		...
		{ cmd = 'time' , description = 'Посмотреть время' ,  text = '/me взглянул{sex} на свои часы с гравировкой LUA MODS и посмотрел{sex} время&/time&/do На часах показано время {get_time}.' , arg = '' , enable = true, waiting = '1.500' , bind = "{}"  },
...
function initialize_commands()
	...
	sampRegisterChatCommand("awanted", function() 
		for i = 1, 10, 1 do
			sampAddChatMessage('[Justice Helper] {ffffff}Данная функция доступна только в платной версии хелпера! Покупать у LUA MODS', message_color)
		end
	end)
...
function sampev.onServerMessage(color,text)
	...
	if (text:find('Bogdan_Martelli%[%d+%]') and getARZServerNumber():find('20')) or text:find('%[20%]Bogdan_Martelli') then
		...
		if text:find('%[20%]Bogdan_Martelli%[%d+%]') then
			...
			text = string.gsub(text, '%[20%]Bogdan_Martelli%[%d+%]', message_color_hex .. '[20]LUA MODS[' .. id .. ']' .. lastColor)
		
		elseif text:find('%[20%]Bogdan_Martelli') then
			-- Случай 1: [20]Bogdan_Martelli
			text = string.gsub(text, '%[20%]Bogdan_Martelli', message_color_hex .. '[20]LUA MODS' .. lastColor)
		
		elseif text:find('Bogdan_Martelli%[%d+%]') then
			-- Случай 3: Bogdan_Martelli[123]
			local id = text:match('Bogdan_Martelli%[(%d+)%]') or ''
			text = string.gsub(text, 'Bogdan_Martelli%[%d+%]', message_color_hex .. 'LUA MODS[' .. id .. ']' .. lastColor)
		elseif text:find('Bogdan_Martelli') then
			-- Случай 3: Bogdan_Martelli
			text = string.gsub(text, 'Bogdan_Martelli', message_color_hex .. 'LUA MODS' .. lastColor)
		end
		return {color,text}
	end
end
					...
					if imgui.BeginTabItem(fa.BARS..u8' RP команды (общие)') then 
						...
						if imgui.Button(fa.CIRCLE_PLUS .. u8' Создать новую команду##new_cmd',imgui.ImVec2(imgui.GetMiddleButtonX(1), 0)) then
							if #commands.commands >= 50 then
								for i = 1, 10, 1 do
									sampAddChatMessage('[Justice Helper] {ffffff}Лимит FREE версии на 50 команд, безлим в платной версии хелпера! Покупать у LUA MODS', message_color)
								end
							else
						...
						if tonumber(settings.player_info.fraction_rank_number) == 9 or tonumber(settings.player_info.fraction_rank_number) == 10 then
							...
							if imgui.Button(fa.CIRCLE_PLUS .. u8' Создать новую команду##new_cmd_9-10', imgui.ImVec2(imgui.GetMiddleButtonX(1), 0)) then
								...
								if #commands.commands_manage >= 15 then
									for i = 1, 10, 1 do
										sampAddChatMessage('[Justice Helper] {ffffff}Лимит FREE версии на 15 лидер.команд, безлим в платной версии хелпера! Покупать у LUA MODS', message_color)
									end
								else
									local new_cmd = {cmd = '', description = '', text = '', arg = '', enable = true, waiting = '1.500', bind = "{}" }
			...
			if imgui.BeginTabItem(fa.FILE_PEN..u8' Заметки') then 
				...
				if imgui.Button(fa.CIRCLE_PLUS .. u8' Создать новую заметку', imgui.ImVec2(imgui.GetMiddleButtonX(1), 0)) then
					if #notes.note >= 5 then
						for i = 1, 10, 1 do
							sampAddChatMessage('[Justice Helper] {ffffff}Лимит FREE версии на 5 заметок, безлим в платной версии хелпера! Покупать у LUA MODS', message_color)
						end
					else
